...
export async function POST(request: Request) {
  ...
  if (channel !== "email" && !recipient.includes("@")) {
    const normalizedRecipient = normalizeBangladeshMobile(recipient);
    if (!normalizedRecipient) return badRequest(getBangladeshMobileValidationMessage("Recipient"));
    recipient = normalizedRecipient;
  }
  const sdk = makeSdkClient(request, hdrs);
  const result = await sdk.sendOtp({ body: { recipient, type: body.type, channel: body.channel } });
