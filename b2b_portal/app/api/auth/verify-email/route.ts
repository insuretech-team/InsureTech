...
export async function POST(request: Request) {
  ...
  const result = await sdk.verifyEmail({ body: { otp_id: body.otp_id, code: body.code } });
  if (!result.response.ok) return NextResponse.json({ ok: false, message: sdkErrorMessage(result) }, { status: result.response.status });
  return NextResponse.json({ ok: true, message: "Email verified", data: result.data }, { status: 200 });
}
