...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const org = (unwrapped.data as Record<string, unknown>)?.organisation as Organisation | undefined;
    return NextResponse.json({ ok: true, organisation: org ? mapOrg(org) : null });
  } catch (err) {
...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    if (normalizedContactPhone) payload.contact_phone = normalizedContactPhone;
