...
export function unwrapGateway<T>(
  ...
  httpStatus?: number
): { ok: true; data: T; meta: GatewayMeta } | { ok: false; message: string; code: string; status: number; retryable: boolean } {
...
export function extractGatewayError(result: unknown): string {
  ...
  if ('data' in r && r.data && typeof r.data === 'object') {
    ...
    if ('success' in d && d.success === false && 'error' in d && d.error && typeof d.error === 'object') {
