...
func main() {
	...
	logger.Info("Ai service database ready",
	...
	// TODO: Initialize ai server when implemented
	// aiServer, err := aipkg.NewAiServer(sqlDB)
	// if err != nil {
	// 	logger.Fatal("Failed to create ai server", zap.Error(err))
	// }
	_ = sqlDB

	// Setup gRPC server
	grpcPort := getEnvOrDefault("AI_GRPC_PORT", "50320")
	...
	// TODO: Register ai service when implemented
	// aiservicev1.RegisterAiServiceServer(grpcServer, aiServer)
	...
	go func() {
		if err := grpcServer.Serve(lis); err != nil {
			logger.Error("Ai gRPC server error", zap.Error(err))
		}
	}()
