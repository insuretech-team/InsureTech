...
// Wrapper entrypoint that delegates to microservices/b2b/cmd/server/main.go.
func main() {
	...
	actualMain := filepath.Join(projectRoot, "backend", "inscore", "microservices", "b2b", "cmd", "server", "main.go")
	logger.Info("Starting B2B service...")
	...
	if err := cmd.Run(); err != nil {
		logger.Errorf("B2B service failed: %v", err)
		os.Exit(1)
	}
}
