...
func main() {
	...
	logger.Info("Starting Beneficiary Service...")
	_ = env.Load()
	...
	logger.Info("Beneficiary service database ready",
	...
	// TODO: Initialize beneficiary server when implemented
	// beneficiaryServer, err := beneficiarypkg.NewBeneficiaryServer(sqlDB)
	// if err != nil {
	// 	logger.Fatal("Failed to create beneficiary server", zap.Error(err))
	// }
	_ = sqlDB

	// Setup gRPC server
	grpcPort := getEnvOrDefault("BENEFICIARY_GRPC_PORT", "50110")
	...
	// TODO: Register beneficiary service when implemented
	// beneficiaryservicev1.RegisterBeneficiaryServiceServer(grpcServer, beneficiaryServer)
	...
	go func() {
		if err := grpcServer.Serve(lis); err != nil {
			logger.Error("Beneficiary gRPC server error", zap.Error(err))
		}
	}()

	logger.Info("Beneficiary service running",
	...
	logger.Info("Shutting down beneficiary service...")
	grpcServer.GracefulStop()
	logger.Info("Beneficiary service stopped")
}
