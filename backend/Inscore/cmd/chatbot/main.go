...
// Wrapper entrypoint that delegates to microservices/chatbot/cmd/server/main.go.
func main() {
	...
	actualMain := filepath.Join(projectRoot, "backend", "inscore", "microservices", "chatbot", "cmd", "server", "main.go")
	...
	if err := cmd.Run(); err != nil {
		logger.Errorf("Chatbot service failed: %v", err)
		os.Exit(1)
	}
}
