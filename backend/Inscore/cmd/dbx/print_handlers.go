...
func handlePrintTable(tableName, targetDB string) {
	...
	if tableName == "" {
		...
		fmt.Println("\n📖 Usage Examples:")
		fmt.Println("   dbx print-table --table=users")
		fmt.Println("   dbx print-table --table=auth.users")
		fmt.Println("   dbx print-table --table=auth.customers --target=backup")
	...
	if err != nil {
		fmt.Printf("❌ Error: %v\n", err)
		fmt.Println("\n💡 Available options:")
		fmt.Println("   • Use 'dbx print-schema' to list all tables")
		fmt.Println("   • Use 'dbx print-schema --schema=" + schemaName + "' to list tables in " + schemaName)
...
func handlePrintTableData(tableName, targetDB string, limit int) {
	...
	if tableName == "" {
		...
		fmt.Println("\n📖 Usage Examples:")
		fmt.Println("   dbx print-table-data --table=users")
		fmt.Println("   dbx print-table-data --table=auth.users")
		fmt.Println("   dbx print-table-data --table=auth.users --limit=50")
		fmt.Println("   dbx print-table-data --table=auth.customers --target=backup")
		fmt.Println("\n💡 Tip: Use 'dbx print-schema' to see available tables")
		return
	}
