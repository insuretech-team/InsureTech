...
// Wrapper entrypoint that delegates to microservices/fraud/cmd/server/main.go.
func main() {
	...
	actualMain := filepath.Join(projectRoot, "backend", "inscore", "microservices", "fraud", "cmd", "server", "main.go")
	...
	if err := cmd.Run(); err != nil {
		logger.Errorf("Fraud service failed: %v", err)
		os.Exit(1)
	}
}
