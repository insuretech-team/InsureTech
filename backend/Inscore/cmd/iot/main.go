...
func main() {
	...
	logger.Info("Iot service database ready",
	...
	// TODO: Initialize iot server when implemented
	// iotServer, err := iotpkg.NewIotServer(sqlDB)
	// if err != nil {
	// 	logger.Fatal("Failed to create iot server", zap.Error(err))
	// }
	_ = sqlDB

	// Setup gRPC server
	grpcPort := getEnvOrDefault("IOT_GRPC_PORT", "50300")
	...
	// TODO: Register iot service when implemented
	// iotservicev1.RegisterIotServiceServer(grpcServer, iotServer)
	...
	go func() {
		if err := grpcServer.Serve(lis); err != nil {
			logger.Error("Iot gRPC server error", zap.Error(err))
		}
	}()

	logger.Info("Iot service running",
