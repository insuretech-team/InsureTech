...
func main() {
	...
	logger.Info("Kyc service database ready",
	...
	// TODO: Initialize kyc server when implemented
	// kycServer, err := kycpkg.NewKycServer(sqlDB)
	// if err != nil {
	// 	logger.Fatal("Failed to create kyc server", zap.Error(err))
	// }
	_ = sqlDB

	// Setup gRPC server
	grpcPort := getEnvOrDefault("KYC_GRPC_PORT", "50090")
	...
	// TODO: Register kyc service when implemented
	// kycservicev1.RegisterKycServiceServer(grpcServer, kycServer)
	...
	go func() {
		if err := grpcServer.Serve(lis); err != nil {
			logger.Error("Kyc gRPC server error", zap.Error(err))
		}
	}()
