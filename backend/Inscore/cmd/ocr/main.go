...
func main() {
	...
	logger.Info("Ocr service database ready",
	...
	// TODO: Initialize ocr server when implemented
	// ocrServer, err := ocrpkg.NewOcrServer(sqlDB)
	// if err != nil {
	// 	logger.Fatal("Failed to create ocr server", zap.Error(err))
	// }
	_ = sqlDB

	// Setup gRPC server
	grpcPort := getEnvOrDefault("OCR_GRPC_PORT", "50270")
	...
	// TODO: Register ocr service when implemented
	// ocrservicev1.RegisterOcrServiceServer(grpcServer, ocrServer)
	...
	go func() {
		if err := grpcServer.Serve(lis); err != nil {
			logger.Error("Ocr gRPC server error", zap.Error(err))
		}
	}()

	logger.Info("Ocr service running",
