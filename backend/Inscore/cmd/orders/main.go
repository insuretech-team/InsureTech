...
// Wrapper entrypoint that delegates to microservices/orders/cmd/server/main.go.
func main() {
	...
	actualMain := filepath.Join(projectRoot, "backend", "inscore", "microservices", "orders", "cmd", "server", "main.go")
	...
	if err := cmd.Run(); err != nil {
		logger.Errorf("Orders service failed: %v", err)
		os.Exit(1)
	}
}
