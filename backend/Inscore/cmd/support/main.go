...
func main() {
	...
	logger.Info("Support service database ready",
	...
	// TODO: Initialize support server when implemented
	// supportServer, err := supportpkg.NewSupportServer(sqlDB)
	// if err != nil {
	// 	logger.Fatal("Failed to create support server", zap.Error(err))
	// }
	_ = sqlDB

	// Setup gRPC server
	grpcPort := getEnvOrDefault("SUPPORT_GRPC_PORT", "50240")
	...
	// TODO: Register support service when implemented
	// supportservicev1.RegisterSupportServiceServer(grpcServer, supportServer)
	...
	go func() {
		if err := grpcServer.Serve(lis); err != nil {
			logger.Error("Support gRPC server error", zap.Error(err))
		}
	}()

	logger.Info("Support service running",
