...
func NewServer(config *Config, svc domain.WorkflowService, interceptors ...grpc.ServerOption) (*Server, error) {
	grpcServer := grpc.NewServer(interceptors...)
	healthServer := health.NewServer()
	handler := NewWorkflowHandler(svc)
...
func (s *Server) registerServices() {
	...
	workflowservicev1.RegisterWorkflowServiceServer(s.server, s.handler)
...
func (s *Server) Start() error {
	...
	appLogger.Infof("workflow-engine gRPC server listening on port %s", s.config.Port)
	s.health.SetServingStatus("workflow", grpc_health_v1.HealthCheckResponse_SERVING)
	s.health.SetServingStatus("insuretech.workflow.services.v1.WorkflowService", grpc_health_v1.HealthCheckResponse_SERVING)
