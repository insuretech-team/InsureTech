...
> **Last Updated:** March 17, 2026 | **Overall Completion: ~60%**
>
...
| `IOrderDataGateway` | `GoOrderDataGateway` | `SyncOrderService.Create/Get/List/InitiatePayment/ConfirmPayment/Cancel/GetOrderStatus` |
...
UnderwritingGrpcService + GoUnderwritingDataGateway + contract tests. Underwriting approval applies loading-adjusted premium back to linked quotation. Underwriting rejection projects quotation status/reason (`REJECTED`). Kafka flow wired: `insuretech.quotation.submitted.v1` consumer seeds underwriting quote and `insuretech.underwriting.decision_made.v1` is published on approve/reject. Risk scoring service and 20+ case score matrix tests are now implemented. Quote side now has `QuotesGrpcService` plus `IQuotationDataGateway` / `GoQuotationDataGateway` over InsuranceService quotation CRUD, a working quotation rehydration path in the domain aggregate, and a dedicated proto mapper for persistence/view mapping. Remaining gaps: dedicated public quote proto, premium calculation query, expiry job, and tests.

**Phase 4 – Policy (✅ gRPC), Orders (✅ gRPC + REST):**
PolicyGrpcService (Create/Get/List/Update/Cancel/Renew/GenerateDocument/Issue) + GoPolicyDataGateway + contract tests + 4 commands + 3 queries. Orders now delegate to Go `sync_order` service via `OrderServiceGrpcClient` plus `IOrderDataGateway` / `GoOrderDataGateway`; C# validates quotation and order transitions before delegating the lifecycle RPCs. The order application handlers and query handlers now target the current Go order-service contract and use a dedicated `OrderViewMapper` to translate gRPC payloads back into application DTOs. Proposal workflow foundation is now present in the proto/schema layer: a paid order can create an insurer-facing `InsuranceProposal`, wait for insurer approval/rejection, then either issue policy or trigger refund. The current runtime gap is that `OrderPaymentConfirmedConsumer` still issues the policy directly; the proposal submission/decision saga is defined but not yet wired end-to-end in PoliSync.
...
### March 17, 2026 Repo Repair Update

This repo repair pass focused on getting `backend/polisync` back to a clean, working state against the current proto contracts and Go gateway services.

**Done in this pass:**

- **ApiHost wiring restored:** `src/PoliSync.ApiHost/Program.cs` now registers REST controllers and activates the gRPC auth/logging/validation interceptors that the host was expected to use.
- **Background consumer repaired:** `src/PoliSync.ApiHost/BackgroundServices/OrderPaymentConfirmedConsumer.cs` was updated to compile against the current order/policy gateway surface and to keep the payment-confirmed → policy-issued projection working.
- **Quotes persistence path repaired:** `src/PoliSync.Quotes/Domain/Quotation.cs`, `src/PoliSync.Quotes/Infrastructure/IQuotationDataGateway.cs`, `src/PoliSync.Quotes/Infrastructure/GoQuotationDataGateway.cs`, `src/PoliSync.Quotes/Infrastructure/QuotationProtoMapper.cs`, and `src/PoliSync.Quotes/Application/Queries/ListQuotationsQueryHandler.cs` were updated so quotation records can be rehydrated from the Go insurance-service contract instead of failing on the half-finished refactor.
- **Orders application layer repaired:** `src/PoliSync.Orders/Application/Commands/CreateOrderCommandHandler.cs`, `CancelOrderCommandHandler.cs`, `ConfirmPaymentCommandHandler.cs`, `InitiatePaymentCommandHandler.cs`, `VerifyPaymentCommandHandler.cs`, `src/PoliSync.Orders/Application/Queries/GetOrderByIdQuery.cs`, `GetOrderByIdQueryHandler.cs`, `ListOrdersQueryHandler.cs`, `src/PoliSync.Orders/Infrastructure/OrderViewMapper.cs`, and `src/PoliSync.Orders/PoliSync.Orders.csproj` were updated so order commands and queries use the current Go `orders-service` contract and DTO mapping.
- **Policy proto mismatch fixed:** `src/PoliSync.Policy/GrpcServices/PolicyGrpcService.cs` was corrected to use the generated renew-policy-tenure RPC contract instead of the stale method/message names.
- **Proposal workflow contract added:** `proto/insuretech/policy/entity/v1/insurance_proposal.proto` now defines the insurer proposal table in `insurance_schema`; `proto/insuretech/insurance/services/v1/insurance_service.proto` now exposes CRUD for proposal persistence; `proto/insuretech/orders/entity/v1/order.proto` now tracks proposal/refund linkage and proposal-specific order statuses; `proto/insuretech/refund/entity/v1/refund.proto` now allows refund linkage to an `order_id` / `proposal_id` for pre-policy rejection refunds; `backend/inscore/microservices/insurance/internal/repository/insurance_proposal_repository.go` and `backend/inscore/microservices/insurance/service/insurance_service.go` now back the new insurance-service CRUD. Generated stubs were regenerated via `scripts/generate.ps1` so the Go models include actual `gorm:"..."` tags.
- **Solution verification completed:** `dotnet build PoliSync.sln` passed, and `dotnet test PoliSync.sln --no-build` passed for the active test suites after the repair work.

**Still pending after this pass:**

- **Quotes:** dedicated public quote proto, premium calculation query, quotation expiry background job, and quote-focused tests are still missing.
- **Orders:** broader happy-path and failure-path tests are still thin; the handler wiring is repaired, but the area still needs deeper coverage and more operational hardening.
- **Proposal saga wiring:** `OrderPaymentConfirmedConsumer` still goes directly to policy issuance. Pending work is to create `InsuranceProposal` on payment confirmation, ingest insurer approve/reject callbacks or events, issue policy only on approval, and trigger refund initiation on rejection.
- **Refund orchestration for proposal rejection:** the refund schema can now point at `order_id` / `proposal_id`, but the public refund service and runtime orchestration still need a first-class order/proposal rejection path.
- **Products tests / integration tests:** `PoliSync.Products.Tests` and `PoliSync.Integration.Tests` still report no discovered tests because their current project configuration excludes the actual test sources.
- **Security/compliance hardening:** RBAC/ABAC enforcement, tenant isolation, audit/KYC/fraud production wiring, masking, rate limiting, retention, and the rest of Phase 8 remain incomplete.
- **Phase 5–7 domain depth:** endorsement, renewal, claims, commission, and refund gRPC surfaces exist, but most business-rule depth, async workflows, and production-grade test coverage are still unfinished.

**Verification notes:**

- Active suites passed on March 17, 2026 after the repair work.
- Remaining build warning observed during the pass: `NU1902` for `OpenTelemetry.Instrumentation.AspNetCore` in `PoliSync.ApiHost`.
- Current order projection note remains true: PoliSync consumes `insuretech.orders.v1.payment_confirmed` and publishes `policy.issued` for the Go order projector.
...
└────────┬─────────┘
      ...
      │ PAID │
      └──┬───┘
         │ SubmitProposalToInsurer()
         ▼
┌────────────────────┐
│ PROPOSAL_SUBMITTED │
└───────┬────────────┘
        │ insurer decision
        ├──────────────► PROPOSAL_APPROVED ──► IssuePolicy() ──► POLICY_ISSUED
        │
        └──────────────► PROPOSAL_REJECTED ──► REFUND_PENDING ──► REFUNDED
                                                             │
                                                             └──► FAILED
...
**Status:** 🟡 Contract + schema foundation added; saga wiring still pending

#### Implementation Steps

**Step 1 — Extend `Order` aggregate and proto state model**
...
public sealed class Order : Entity
{
    ...
    public Result SubmitProposal(Guid proposalId)
    {
        if (Status != OrderStatus.Paid)
            return Result.Fail("INVALID_TRANSITION", $"Cannot submit proposal from {Status}");
        ProposalId = proposalId;
        Status = OrderStatus.ProposalSubmitted;
        return Result.Ok();
    }

    public Result ApproveProposal()
    {
        if (Status != OrderStatus.ProposalSubmitted)
            return Result.Fail("INVALID_TRANSITION", $"Cannot approve proposal from {Status}");
        Status = OrderStatus.ProposalApproved;
        return Result.Ok();
    }

    public Result RejectProposal(Guid refundId)
    {
        if (Status != OrderStatus.ProposalSubmitted)
            return Result.Fail("INVALID_TRANSITION", $"Cannot reject proposal from {Status}");
        RefundId = refundId;
    ...
    {
        if (Status != OrderStatus.ProposalApproved)
            return Result.Fail("INVALID_TRANSITION", "Order must be proposal-approved to link policy");
        PolicyId = policyId;
...
public enum OrderStatus
{
    ...
    ProposalSubmitted = 7, ProposalApproved = 8,
    ProposalRejected = 9, RefundPending = 10, Refunded = 11
}
```
...
On payment gateway callback → `order.ConfirmPayment()` → create `InsuranceProposal` via `InsuranceService.CreateInsuranceProposal`.

**Step 4 — Proposal decision handling:**
On insurer approval callback/event → mark proposal approved → dispatch `IssuePolicyCommand`.
On insurer rejection callback/event → create refund linked to `order_id` and `proposal_id` → transition order to `REFUND_PENDING` / `REFUNDED`.
...
### SyncOrderService (:50142) — Order Lifecycle Persistence [7 RPCs]
...
Current implementation note: the Go `sync_order` service uses versioned Kafka topics for the order→policy projection path. PoliSync currently consumes `insuretech.orders.v1.payment_confirmed` and publishes `policy.issued` for the Go order projector. The new proposal architecture adds a missing insurer-decision stage, but the event wiring for proposal submit/approve/reject is still pending.
...
| `insuretech.proposal.submitted.v1` | polisync order/policy saga | insurer integration | ❌ Planned |
| `insuretech.proposal.approved.v1` | insurer integration | polisync policy saga | ❌ Planned |
| `insuretech.proposal.rejected.v1` | insurer integration | polisync refund saga | ❌ Planned |
...
### `insurance_schema`
`products`, `product_plans`, `pricing_configs`, `riders`, `quotations`, `orders`, `insurance_proposals`, `policies`, `nominees`*(PII)*, `policy_riders`, `endorsements`, `renewal_schedules`, `renewal_reminders`, `grace_periods`, `health_declarations`, `underwriting_decisions`, `claims`, `claim_documents`, `claim_approvals`, `settlements`, `refunds`
