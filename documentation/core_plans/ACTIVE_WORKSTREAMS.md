...
- Go sync_order service/data layer in `backend/inscore/microservices/orders/` (key: sync_order in services.yaml) — typed event consumers
...
- Kafka topics renamed to canonical `insuretech.payment.v1.*` and `insuretech.orders.v1.*` format
