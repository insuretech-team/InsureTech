...
PolicyGrpcService (Create/Get/List/Update/Cancel/Renew/GenerateDocument/Issue) + GoPolicyDataGateway + contract tests + 4 commands + 3 queries. Orders now delegate to Go `sync_order` service via `OrderServiceGrpcClient` plus `IOrderDataGateway` / `GoOrderDataGateway`; C# validates quotation and order transitions before delegating the lifecycle RPCs. `OrderPaymentConfirmedConsumer` now issues the policy inside PoliSync and publishes a `policy.issued` projection so Go `sync_order` service can link `policy_id` and move the order to `POLICY_ISSUED`.
