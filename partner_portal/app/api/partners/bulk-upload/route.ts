/**
 * /api/partners/bulk-upload  POST
 *
 * Accepts multipart/form-data:
 *   - file : Excel (.xlsx) or CSV file
 *
 * Proxies directly to the gateway POST /v1/partners/bulk-upload
...
export async function POST(request: Request) {
  try {
    ...
    const xPartnerId =
      hdrs?.partnerId ?? request.headers.get("x-partner-id") ?? "";
    const xUserId =
    ...
    if (xPartnerId) forwardHeaders["x-partner-id"] = xPartnerId;
    ...
    const gatewayRes = await fetch(
      `${getBaseUrl()}/v1/partners/bulk-upload`,
      {
        method: "POST",
