...
import {
  ...
  type PurchaseOrders,
} from "@/components/dashboard/purchase-orders/data-table/columns";
    ...
    <DashboardLayout>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
        {purchaseOrders.map((order) => (
          <PurchaseCard key={order.id} {...order} />
        ))}
      </div>

      <div className="space-y-4 py-4">
        <DataTable columns={purchaseOrderColumns} data={data} loading={false} />
      </div>
    </DashboardLayout>
  );
};
