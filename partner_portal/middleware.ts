...
const ROLE_GUARDS: Array<{ prefix: string; allowedRoles: string[] }> = [
  { prefix: "/partners", allowedRoles: ["SYSTEM_ADMIN", "PARTNER_ADMIN"] },
  {
    prefix: "/agents",
    allowedRoles: ["SYSTEM_ADMIN", "PARTNER_ADMIN", "PARTNER_FOCAL_PERSON"],
  },
  {
    prefix: "/claims",
    allowedRoles: [
  ...
  { prefix: "/commission", allowedRoles: ["SYSTEM_ADMIN", "PARTNER_ADMIN"] },
  {
    prefix: "/settings",
    allowedRoles: ["SYSTEM_ADMIN", "PARTNER_ADMIN"],
  },
];
...
export function middleware(request: NextRequest) {
  ...
  if (hasSessionCookie && pathname === "/login") {
    ...
    const dest = role === "SYSTEM_ADMIN" ? "/partners" : "/";
  ...
  if (hasSessionCookie) {
    ...
    if (role) {
      ...
      if (guard && !guard.allowedRoles.includes(role)) {
        ...
        const fallback = role === "SYSTEM_ADMIN" ? "/partners" : "/";
