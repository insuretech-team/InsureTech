 ...
 * x-partner-id, x-tenant-id headers to inject into backend SDK calls.
 *
 ...
 * cookie set at login time, and the partnerId from the `portal_partner_id` cookie
 * (also set at login for partner users).
 *
 * Authorization model:
 * The backend authz interceptor needs x-portal=PORTAL_SYSTEM to route the
 * request to the system:root Casbin domain (no x-partner-id required).
 * Partner users get x-portal=PORTAL_PARTNER with their x-partner-id for scoped access.
 */
 ...
 * SYSTEM_ADMIN → PORTAL_SYSTEM (super admin, no partner context required)
 * Everything else → PORTAL_PARTNER (partner admin / focal person / agent / viewer)
 */
 ...
 *   portal_role        — e.g. "SYSTEM_ADMIN" | "PARTNER_ADMIN" | "PARTNER_FOCAL_PERSON" | "PARTNER_AGENT"
 *   portal_user_id     — user ID for x-user-id (optional, may be empty)
 *   portal_partner_id  — partner/org ID for x-partner-id (partner users only)
 *
 * If portal_role is missing, we fall back to PORTAL_PARTNER (safest non-elevated default).
...
export async function resolvePortalHeaders(request: Request): Promise<PortalHeaders | null> {
  ...
  const role = extractCookie(cookieHeader, "portal_role") || "PARTNER_ADMIN";
  const userId = extractCookie(cookieHeader, "portal_user_id");
  const partnerId = extractCookie(cookieHeader, "portal_partner_id");
  ...
  return { portal, userId, partnerId, tenantId };
}
